...
class JobControllerTest extends WebTestCase
{
    public function getMostRecentProgrammingJob()
    {
      ...
      $query = $em->createQuery('SELECT j from JobeetBundle:Job j LEFT JOIN j.category c WHERE c.slug = :slug AND j.expiresAt > :date ORDER BY j.expiresAt DESC');
    ...
    public function getExpiredJob()
    {
      ...
      $query = $em->createQuery('SELECT j from JobeetBundle:Job j WHERE j.expiresAt < :date');     
      $query->setParameter('date', date('Y-m-d H:i:s', time()));
      $query->setMaxResults(1);
      return $query->getSingleResult();
    }
    ...
    public function testPublishJob()
    {
      ...
      $query = $em->createQuery('SELECT count(j.id) from JobeetBundle:Job j WHERE j.position = :position AND j.isActivated = 1');
    ...
    public function testIndex()
    {
      ...
      //$this->assertEquals('Epfc\JobeetBundle\Controller\JobController::indexAction', $client->getRequest()->attributes->get('_controller'));
      ...
      //$this->assertTrue($crawler->filter('.category_design .more_jobs')->count() == 0); // true
      //$this->assertTrue($crawler->filter('.category_programming .more_jobs')->count() == 1); // true
      ...
      $this->assertEquals('Epfc\JobeetBundle\Controller\JobController::showAction', $client->getRequest()->attributes->get('_controller'));
    ...
    public function testJobForm()
    {
        ...
        $crawler = $client->request('GET', '/job/new');
        $this->assertEquals('Epfc\JobeetBundle\Controller\JobController::newAction', $client->getRequest()->attributes->get('_controller'));

        $form = $crawler->selectButton('Preview your job')->form(array(
          'job[company]'      => 'Sensio Labs',
          'job[url]'          => 'http://www.sensio.com/',
          //'job[file]'         => __DIR__.'/../../../../../../web/public/images/sensio-labs.gif',
          'job[position]'     => 'Developer',
          'job[location]'     => 'Atlanta, USA',
          'job[description]'  => 'You will work with symfony to develop websites for our customers.',
          'job[howToApply]' => 'Send me an email',
        ...
        $client->submit($form);
        $this->assertEquals('Epfc\JobeetBundle\Controller\JobController::createAction', $client->getRequest()->attributes->get('_controller'));

        $client->followRedirect();
        $this->assertEquals('Epfc\JobeetBundle\Controller\JobController::previewAction', $client->getRequest()->attributes->get('_controller'));

        $kernel = static::createKernel();
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');

        $query = $em->createQuery('SELECT count(j.id) from JobeetBundle:Job j WHERE j.location = :location AND j.isActivated IS NULL AND j.isPublic = 0');
    ...
    public function testExtendJob()
    {
      ...
      $kernel->boot();
      $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
      $job = $em->getRepository('JobeetBundle:Job')->findOneByPosition('FOO5');
    ...
    public function testCompleteScenario()
    {
        ...
        $form = $crawler->selectButton('Create')->form(array(
            'epfc_jobeetbundle_job[field_name]'  => 'Test',
        ...
        $form = $crawler->selectButton('Update')->form(array(
            'epfc_jobeetbundle_job[field_name]'  => 'Foo',
            // ... other fields to fill
        ));
